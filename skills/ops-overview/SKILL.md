---
name: ops-overview
description: 输出平台经营大盘——用户数、Agent 数、近 7 天活跃、创建 Agent 转化率、Credit 消耗/发放、邀请码使用率。当用户要求「看经营数据」「平台有多少用户」「消耗了多少积分」「Credit 情况」时使用。
---
...
- **Agent**：总数、活跃数、昨日决策量
- **近 7 天活跃**：活跃用户数、活跃 Agent 数、交易额（USDT）
- **转化**：创建 Agent 的用户数、转化率（创建 Agent 用户 / 总用户）
- **Credit**：充值 / 扣费消耗 / 奖励 / 发放 / 过期 各自总额与笔数（金额按 credit 单位原样呈现，注明是「总额」还是「净额」）
- **邀请码**：发放/使用/使用率
- **一句话洞察**：对比本周 vs 上周新增趋势、近 7 天活跃与转化、Credit 净流入/流出方向
